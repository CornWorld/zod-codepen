...
builtinHandlers.set("number", (schema, ctx) => {
  ...
  // 判断是否使用语义化方法
  const useSemanticMethods =
    ctx.options.optimizations?.semanticMethods !== false;
  const useScientific = ctx.options.optimizations?.scientificNotation !== false;

  for (const check of checks) {
    const value = (check.value ?? check.minimum ?? check.maximum) as
      | number
      | undefined;
    switch (check.kind) {
      case "min":
        ...
        if (useSemanticMethods) {
          ...
          if (value === 0 && check.inclusive === false) {
            ...
            checks.some(
              (c) =>
                c.kind === "max" &&
                (c.value ?? c.maximum) === Number.MAX_SAFE_INTEGER &&
                c.inclusive === true,
            )
          ) {
...
builtinHandlers.set("function", (schema, ctx) => {
  ...
  if (argsTuple && ctx.adapter.isZodSchema(argsTuple)) {
    ...
    if (argsItems && Array.isArray(argsItems) && argsItems.length > 0) {
      const argsStr = argsItems
        .map((arg: unknown) => ctx.serialize(arg))
        .join(", ");
      result += `.args(${argsStr})`;
    }
  }
