...
- **静态提取模式**：不执行用户代码，纯文本解析 schema 文件（适合 Edge/Cloudflare Workers）
...
- **导出**：
  - `zodDecoupling` — 完整插件（自动生成 + 别名，运行时模式）
  - `zodDecouplingStatic` — 静态提取插件（纯文本解析，不执行用户代码）
  - `zodDecouplingAlias` — 仅别名插件
  - `generateSchemas` — 运行时独立生成函数
  - `generateSchemasFromSource` — 静态独立生成函数（接收源码字符串）
...
#### 模式 3：静态提取模式（适合 Edge/Cloudflare Workers）
...
1. 读取 `schemaEntry` 文件为纯文本
2. 用 TypeScript compiler API 解析 AST（不执行代码）
3. 生成纯 Zod 代码到 `outputPath`
4. 设置 Vite alias 替换导入

**适用场景**：schema 文件依赖 drizzle-orm、pg 等重运行时模块，无法在构建环境中 `await import()`。
...
#### `generateSchemasFromSource(options): Promise<void>`

静态生成函数，从 TypeScript 源码文本中提取 schema，不执行用户代码。

**选项**：

```typescript
interface GenerateFromSourceOptions {
  source: string; // TypeScript 源码文本
  outputPath: string; // 输出文件路径
  filter?: (name: string, ir: IRNode) => boolean; // 过滤器
  includeTypes?: boolean; // 是否导出类型（默认 true）
  header?: string; // 自定义文件头
  codegenOptions?: CodegenOptions; // codegen 选项
  verbose?: boolean; // 详细日志
}
```
...
- **本模块 6 个测试文件，64 个测试**：
  - `adapters.test.ts`（22 个测试）— 适配器工厂测试
  - `static-extraction.test.ts`（7 个测试）— 静态提取管线测试
  - `plugin.test.ts`（13 个测试）— 插件配置与行为测试
  - `generate-schemas.test.ts`（12 个测试）— generateSchemas 测试
  - `e2e.test.ts`（5 个测试）— 端到端集成测试
  - 包括 drizzle-zod fixture 测试
- **验证方式**：
  - 在真实 Vite 项目中测试别名替换
  - 检查生成文件的正确性
  - 验证构建产物体积减少
...
- **独立脚本模式**：更灵活，可在 Vite 外部运行（如 CI/CD），不受构建时导入限制
- **静态提取模式（zodDecouplingStatic）**：不需要执行用户代码，直接解析 AST，适合 schema 文件依赖重运行时的场景
- 一体化插件（zodDecoupling）适合简单场景
...
当 schema 文件引用了构建环境中无法执行的依赖（如 drizzle-orm、pg）时，使用 `zodDecouplingStatic`。它将源码当纯文本解析，完全不执行用户代码，可以在任何构建环境中运行。
...
- `src/index.ts`：完整插件实现
  - 适配器工厂（v3/v4）
  - `generateSchemas`（运行时）
  - `generateSchemasFromSource`（静态 AST）
  - `zodDecouplingAlias` 插件
  - `zodDecoupling` 完整插件（运行时）
  - `zodDecouplingStatic` 完整插件（静态 AST）
